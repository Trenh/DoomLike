...
idl: ## Generate code from IDL.
	...
	cd externals/bgfx/scripts && ../$(GENIE) idl
...
rebuild-shaders:
	$(MAKE) -R -C externals/bgfx/examples rebuild

assets: # Build assets.
	$(NINJA) -C scripts

analyze:
	cppcheck externals/bgfx/src/
	cppcheck externals/bgfx/examples/

docs:
	doxygen externals/bgfx/scripts/bgfx.doxygen
